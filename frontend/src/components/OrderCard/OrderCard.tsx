...
const OrderCard: React.FC<OrderCardProps> = ({ order }) => {
    ...
    const onClickEdit = () => {
        ...
        dispatch(setModalTitle('Редактировать заказ'));
    ...
    return (
        <Card className={"border-0 rounded-3 shadow-sm"}>
            <Card.Header className={"d-flex p-3"} style={{ backgroundColor: "#4285B4" }}>
                <p className={"fw-bold"}>ID заказа: {order.id}</p>
                <p className={"text-muted ms-auto"}>Дата: {dateFormat}</p>
            </Card.Header>
            <Card.Body>
